...
def mfep_with_bl(x, y, s_ph, fenergy, psi_abs, b, j_x, j_y, bl, x_ax, x0):
    ...
    gs = gridspec.GridSpec(4, 4, wspace=0.08, width_ratios=[1, 1, 1, 0.08], height_ratios=[0.005,2,2,0.5])
    ...
    ax.quiver(X, Y, U, V, color=a_color, scale=9*np.max(np.sqrt(U**2+V**2)), width=a_width)
    ...
    ax.plot(np.abs(x0-x[-1, -1]), fenergy, '.', color='r', markersize=2, alpha=0.9, label="MFEP")
    ax.set_xlabel(r"$x$")
    ax.set_ylabel(r"$F(x)$")
    ax.legend()
...
def mfep_psi_B(x, y, fenergy, psi_abs, b, j_x, j_y):
    ...
    ax[0, 0].quiver(X, Y, U, V, color=[0.9, 0.9, 0.9, 0.4], scale=18*np.max(np.sqrt(U**2+V**2)))
    ...
    ax[0, 1].quiver(X, Y, U, V, color=[0.9, 0.9, 0.9, 0.4], scale=18*np.max(np.sqrt(U**2+V**2)))
    ...
    ax[0, 2].quiver(X, Y, U, V, color=[0.9, 0.9, 0.9, 0.4], scale=18*np.max(np.sqrt(U**2+V**2)))
...
def complex_plot(x, y, psi_abs, psi_theta):
    ...
    fig, ax = plt.subplots(dpi=300)
    ax.imshow(rgba, origin = "lower", interpolation="nearest", extent=[x.min(), x.max(), y.min(), y.max()],)
...
def complex_plot_3D(xx, yy, psi_abs, psi_theta):
    ...
    cmap = cmocean.cm.phase(theta_deg)
...
def vector_potential_plot(x, y, ax, ay, a_max, a_min, title=""):
    fig, axes = plt.subplots(nrows=1, ncols=2, sharex=True, sharey=False, dpi=300, figsize=(11,7))
...
def current_animation(F, x, y, j_x, j_y, scale, borders=None):
    ...
    anim = animation.FuncAnimation(fig, update_quiver, frames=F, interval=250, blit=False)
    fig.tight_layout()
...
def magnetic_field_animation(F, x, y, b, borders=None):
    ...
    anim = animation.FuncAnimation(fig, update_contourf, frames=F, interval=250, blit=False)
    fig.tight_layout()
...
def complex_field_animation(F, x, y, psi_abs, psi_theta, borders=None):
    ...
    fig, ax = plt.subplots(dpi=300)
    im = ax.imshow(rgb[0], origin = "lower", interpolation="lanczos", extent=[x.min(), x.max(), y.min(), y.max()],)
    ...
    anim = animation.FuncAnimation(fig, update_cplot, frames=F, interval=250, blit=False)
    fig.tight_layout()
